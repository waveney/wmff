<?php
  ...
  echo "</head><body>\n";
  echo '<a href="/" rel="bookmark"><h1>Wimborne Minster Folk Festival | 8, 9, 10 June 2018</h1></a>';
...
<ul class="rslides" id="slider1">
  <li><a href="/info/trade" rel="bookmark"><img src="/images/Highstreet-Traders-2016.jpg"></a></li>
...
$Photo = "/images/Hobos-Morris-2016.jpg";
  ...
  if ($ans) {
    ...
    echo "<a href=/int/ShowDance.php?sidenum=" . $stuff['SideId'] . " rel=bookmark>";
  ...
  echo "<div class=article>";
  echo "<a href=/line-up/dance rel=bookmark>";
...
<div class="article">
<a href="/tickets" rel="bookmark">
<h2 class="articlettl">Buy Tickets and Day/Weekend Passes</h2>
...
<div class="article">
<a href="/info/trade" rel="bookmark">
...
<?php 
  ...
  $ans = $db->query("SELECT s.* FROM Sides s, ActYear y WHERE s.IsAnAct=1 AND s.SideId=y.SideId AND y.Year=$THISYEAR AND s.Photo!='' AND y.YearState>0 " . 
			" AND s.Importance!=0 ORDER BY RAND() LIMIT 1");
  if ($ans) {
    $stuff = $ans->fetch_assoc();
    if ($stuff['SideId'] == $p['SideId']) $stuff = $ans->fetch_assoc();
    echo "<div class=article>";
    echo "<a href=/int/ShowMusic.php?sidenum=" . $stuff['SideId'] . " rel=bookmark>";
    echo "<h2 class=articlettl>" . $stuff['Name'] . "</h2>";
    echo "<img class=articleimg src=" . $stuff['Photo'] . " alt=Wimborne Minster Folk Festival /></a>";
    echo "<p class=articletxt>" . $stuff['Description'] . "\n";
    echo "</div>";
  ...
  echo "<div class=article>";
  echo "<a href=/line-up/music rel=bookmark>";
...
<!--
<div class="article">
<a href="/info/camping" rel="bookmark">
...
Booking should be available soon.<p>
</div>
-->
...
<?php
  ...
  echo "Wimborne Minster Folk Festival would not be possible without the amazing help and generosity of the following companies and organisations: ";
  echo '<a class="button" style="color:#FFFFFF;" href="/info/sponsorship" rel="bookmark">Become a <strong>Festival Sponsor</strong></a><p>';
