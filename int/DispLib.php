...
function Get_Imps(&$e,&$imps,$clear=1,$all=0) {
  ...
  for($i=1;$i<5;$i++) {
	...
	$s = Get_Side($ee);  
	if ($s && ($all || $ets >1 || ($ets==1 && Access('Participant','Side',$s)))) $imps[$big?0:$s['Importance']][] = $s; }; };
    if (isset($e["Act$i"]))  { if ($ee = $e["Act$i"])   { 
	$s = Get_Side($ee); 
	if ($s && ($all || $ets >1 || ($ets==1 && Access('Participant','Act',$s)))) $imps[$big?0:$s['Importance']][] = $s; }; };
    if (isset($e["Other$i"])){ if ($ee = $e["Other$i"]) { 
	$s = Get_Side($ee);  
	if ($s && ($all || $ets >1 || ($ets==1 && Access('Participant','Other',$s)))) $imps[$big?0:$s['Importance']][] = $s; }; };
  }
}
