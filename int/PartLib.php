...
function Get_Other_People($y=0) { // year 0 all current year, -1 all - no year info, year>1 all that year
  global $db,$YEAR;
  if ($y == 0) {
    $res = $db->query("SELECT o.*, y.* FROM OtherPart o, OtherPartYear y WHERE o.OtherId=y.Other AND y.Year=$YEAR ORDER BY Name");
  } else if ($y < 0) {
    $res = $db->query("SELECT o.*, y.* FROM OtherPart o LEFT JOIN OtherPartYear y ON o.OtherId=y.Other AND y.Year=$YEAR ORDER BY Name");
  } else {
    $res = $db->query("SELECT o.*, y.* FROM OtherPart o, OtherPartYear y WHERE o.OtherId=y.Other AND y.Year=$y ORDER BY Name");
  }
  if ($res) {
...
function Get_Other_People_Day($day) { 
  global $db,$YEAR;
  $res = $db->query("SELECT o.*, y.* FROM OtherPart o, OtherPartYear y WHERE o.OtherId=y.Other AND y.Year=$YEAR AND y.$day=1 ORDER BY Name");
  if ($res) {
    while ($o = $res->fetch_assoc()) { $othrs[$o['OtherId']] = $o; };
    return $othrs;
  }
