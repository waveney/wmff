<?php
  ...
  echo "<h2 class=maintitle>Music Line-up</h2>";
  if ($YEAR < $PLANYEAR) {
  ...
  $SideQ = $db->query("SELECT s.*, y.* FROM Sides AS s, ActYear AS y " .
           "WHERE s.SideId=y.SideId AND y.year=$YEAR AND y.YearState>=" . $Book_State['Booking'] . " AND s.IsAnAct=1 AND y.ReleaseDate<$now ORDER BY s.Importance DESC, s.SN");
  ...
  if ($Prev >= $ET['FirstYear']) {
    echo "<a href=/LineUpMusic.php?Y=$Prev>Music Line Up $Prev</a></b><p>";
  }
