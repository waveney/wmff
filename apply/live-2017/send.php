...
$getesig = ucwords($getesig);
  ...
  if(empty($getcontactname) | empty($getaddress) | empty($getemail) | empty($getphone) | empty($getperformername) | empty($getperformernumber) | empty($getperformerage) | empty($getperformerinst) | empty($getperformergenre) | empty($getperformerdemo))
  {
...
mysql_query("INSERT INTO wmfflive (id, contactname, address, email, phone, performername, performernumber, performerage, performerinst, performergenre, performerdemo, squarerecords, whitehart, risingsun, qe, tech, esig, status, applydate, festivalyear)
VALUES ('$nextnumber', '$getcontactname', '$getaddress', '$getemail', '$getphone', '$getperformername', '$getperformernumber', '$getperformerage', '$getperformerinst', '$getperformergenre', '$getperformerdemo', '$getsquarerecords', '$getwhitehart', '$getrisingsun', '$getqe', '$gettech', '$getesig', 'unconfirmed', '$getdatetime', '2017')");
...
<tr><th colspan=\"2\" style=\"background-color:#a30046; color:#FFFFFF; text-align:left; padding:10px 5px; font-size:18px;\">Live & Loud Application Form</th></tr>
...
<tr><td>Demo:</td><td><a href=\"$getperformerdemo\" target=\"_blank\"><strong>$getperformerdemo</strong></a></td></tr>
<br />
<tr><th colspan=\"2\" style=\"background-color:#a30046; color:#FFFFFF; text-align:left; padding:10px 5px; font-size:18px;\">Preferred Audition Date</th></tr>
<tr><td>Square Records (22 April):</td><td><strong>$getsquarerecords</strong></td></tr>
<tr><td>White Hart (28 April):</td><td><strong>$getwhitehart</strong></td></tr>
<tr><td>Rising Sun (4 May):</td><td><strong>$getrisingsun</strong></td></tr>
<tr><td>QE School (19 May):</td><td><strong>$getqe</strong></td></tr>
...
Your application for Wimborne Live & Loud 2017 has been received. We will be in touch soon.

Join us at the Grand Finals from 7pm on 26 May at Allendale Centre, Wimborne! Tickets are a fiver from WimborneFolk.co.uk/tickets
...
$headers .= "Content-Type: text/html; charset=ISO-8859-1\r\n";
  ...
  mail( "james@wimbornefolk.co.uk, emily@wimbornefolk.co.uk", "Live & Loud Application Form",
  $message, "$headers" );

  mail( "$getemail", "Wimborne Minster Folk Festival | Auto Response",
  $autoreply, "From: Emily@wimbornefolk.co.uk" );

echo "<h2 class=\"maintitle\">Thank You, $getcontactname!</h2><p>Your Live & Loud Application Form has been sent. We will be in touch with you soon.</p><p>We have also sent you confirmation by email that your Application Form has been sent (please check your spam inbox), if you have not received the automated response, the email address you provided may have been mis-spelt, so please contact us directly on <a href=\"mailto:emily@wimbornefolk.co.uk\"><strong>emily@wimbornefolk.co.uk</strong></a>.</p>

<h2 class=\"maintitle\">The Grand Finals</h2>
<p>Join us from <strong>7pm on 26 May 2017</strong> at:</p>
<p>Allendale Centre
<br />Wimborne
<br />BH21 1AS
<br />
<br />Tickets &#163;5.00 per person
<br />(+ booking fee if bought in advance)
<br /><a href=\"http://ticketsource.co.uk/wimbornefolk\" target=\"_blank\"><strong>Buy Online Now</strong></a></p>";
?>
